...
基于 nezha 官方镜像，支持将数据**加密**后自动同步到 **S3 存储桶** 或 **WebDAV 网盘**，特别适用于没有持久化存储卷的云服务、PaaS 平台。本项目默认开启了双架构（`linux/amd64`, `linux/arm64`）支持。

## 💡 工作原理

```text
容器启动 → 从 S3/WebDAV 恢复数据 → 启动 nezha 服务 → 后台每 N 分钟自动备份数据到远端
```
...
4. **启动服务**: 容器将会在拉取云端数据后，在 `8080` 端口开启 nezha 面板服务。
