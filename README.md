# nezha - S3/WebDAV 数据持久化版

基于 nezha 官方镜像，支持将数据**加密**后自动同步到 **S3 存储桶** 或 **WebDAV 网盘**，特别适用于没有持久化存储卷的云服务、PaaS 平台（如 Zeabur、Render、Railway 等）。本项目默认开启了双架构（`linux/amd64`, `linux/arm64`）支持。

## 💡 工作原理

```text
容器启动 → 检查运行模式 (云端/本地)
 ├─ 云端模式: 从 S3/WebDAV 恢复数据 → 启动 nezha 服务 → 后台每 N 分钟自动备份增量数据到远端
 └─ 本地模式: 跳过 rclone 同步 → 直接启动 nezha 服务 (适用于有持久化存储卷的环境)
```

1. **启动时恢复**: 容器启动阶段，使用 `rclone copy` 优先从远端拉取历史备份到本地 `/dashboard/data` 目录。
2. **定时备份**: 通过后台常驻脚本循环 `rclone sync`，单向将本地变化的数据同步到远端网盘。
3. **数据加密**: 可选 AES-256 高强度加密，实现文件名及文件内容的完全加密后再离开本地。
4. **本地兼容**: 完美兼容传统 VPS 或提供持久化存储的 PaaS，无需远端同步也可独立运行。

## ⚙️ 环境变量设置

| 变量名 | 必填 | 说明 | 默认值 |
|--------|------|------|--------|
| `STORAGE_TYPE` | ❌ | 存储类型: `s3`、`webdav` 或 `local` | `local` |
| `SYNC_INTERVAL` | ❌ | 同步间隔时长（分钟） | `5` |

> 💡 **提示**：如果 `STORAGE_TYPE` 不填或设置为 `local`，容器将以纯净本地模式运行，跳过所有远端同步逻辑。
...
在类似 Zeabur、Railway、Render 等 PaaS 平台部署时极其简单：

1. **镜像拉取**: 指定镜像为 `ghcr.io/workerspages/nezha-oss:latest` 或 `docker.io/workerspages/nezha-oss:latest`。
2. **端口设置** ⚠️: 面板主程序默认监听 **`8008`** 端口。如果遇到 `502 Bad Gateway` 错误，**请务必在 PaaS 平台的网络/域名设置中，将目标容器端口强制指定为 `8008`**。
3. **注入变量**: 填补上述的环境变量表，依据您使用的存储方案分配 S3 或是 WebDAV 的密钥信息。
4. **启动服务**: 容器将会在拉取云端数据后，在 `8008` 端口开启 nezha 面板服务。
...
当您修改程序代码并 Push 后，系统会自动采用多阶段构建 (Multi-Stage Build) 和 `Docker Buildx` 引擎编译多平台并发版（涵盖 `linux/amd64` 和 `linux/arm64`）：
